...
*   **Icons**: Lucide React
*   **Styling**: Vanilla CSS (CSS variables, flexbox, grid, and glassmorphic panels)
...
*   **Sensitive Configs**: Never commit `.env` or raw credential files to public version control repositories.
*   **Gitignore**: Ensure `.env` is explicitly included in your `.gitignore` file.
*   **Session Clearance**: Logging out clears `authToken` and `authUser` from `localStorage` immediately.
